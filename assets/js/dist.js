...
},{"github-api":35}],2:[function(require,module,exports){
'use strict';
...
function getTabData(file){

   return new Promise(function (resolve, reject){
    

      //disable it as it will overwrite the merge
      //jsnData.variables = table.getData(); 

      var content = JSON.stringify(jsnData);

      console.log('test', content, file);
      // no need to decode any more
      //content = window.atob(content);
      //content = atob(content.replace(/^(.+,)/, '').replace(/\s/g,''));
      console.log('test2');
...
function uploadFiles(files, commitTitle) {
   ...
   //
   console.log(files, 'inupload')
   var filesPromises = [].map.call(files, getTabData);

   return Promise
      .all(filesPromises)
      .then(function(files) {
         console.log('xum', files);
         return files.reduce(
...
document.querySelector('form').addEventListener('submit', function (event) {
   ...
   var giturl=baseurl + "jsonfiles/cmor/" + fileJson;

   console.log(giturl);

   //check if it is the merge conflict case
   console.log('merge', cannotMerge);
   //
   console.log(JSON.stringify(jsnData), JSON.stringify(jsnData).includes("loc:"));
   if (JSON.stringify(jsnData).includes('loc:')){
       console.log('Please solve conflicts first');
   }
   else{
       cannotMerge = 0;
       //var giturl= "https://cors-anywhere.herokuapp.com/https://comclieco.github.io/cmorjson/jsonfiles/cmor/test2.json";
       var jqxhr = $.getJSON(giturl, function(data){
          console.log("in merging");
          jsnData.variables.forEach( (jsn, i) => {
              //console.log(jsnData);
              if ( jsn.relationship.trim() !== data.variables[i].relationship.trim() ){

                  console.log('diff', jsn.relationship.trim(), data.variables[i].relationship.trim())

                  //if( jsn.relationship.trim() == '' ){
                  //    console.log("merge from git");
                  //    jsn.relationship = data.variables[i].relationship;
                  //    jsn.confidence   = data.variables[i].confidence;
                  //}
                  //else if( data.variables[i].relationship.trim() == '') {
                  if( data.variables[i].relationship.trim() == '') {
                      console.log('use local');
                      console.log("merge to git");
                  }
                  else{   // conflict
                      if (preCommit == 1){
                         if (precommitData.variables[i].relationship.trim() !== data.variables[i].relationship.trim()){
                            jsn.relationship = 'loc:' + jsn.relationship + 'git:' + data.variables[i].relationship;
                            console.log("merge conflict", jsn.relationship);
                            jsn.confidence = -100;
                            cannotMerge = 1;
                         }
                      }
       ...
       .fail(function(){
            console.log("No file on repo, directly commit");
       });

       jqxhr.always(function(){
          //upload files:
          if (cannotMerge == 0){
             //var files= ['jsonfiles/cmor/test2.json'];
             var files = [];

             files.push('jsonfiles/cmor/'.concat(fileJson));

             $('.git-commit').css("display", "inline-block");
             ...
             uploadFiles(files, commitTitle)
                .then(function() {
                   ...
                   preCommit = 0;
                   precommitData = {};
                   //update local table after merging
                   table.setData(jsnData.variables);     
                })
                .catch(function(err) {
          ...
          {
               alert ('Conflicts, please look at the red rows');
               table.setData(jsnData.variables);     
          }
       });
   }
...
},{"./github":1}],3:[function(require,module,exports){
...
},{"./adapters/http":4,"./adapters/xhr":4,"./helpers/normalizeHeaderName":25,"./utils":28,"_process":49}],19:[function(require,module,exports){
'use strict';
...
},{"base64-js":29,"ieee754":46,"isarray":31}],31:[function(require,module,exports){
...
},{"./debug":33,"_process":49}],33:[function(require,module,exports){
...
},{"ms":48}],34:[function(require,module,exports){
'use strict';
...
},{"./Requestable":42,"buffer":30,"debug":32,"js-base64":47,"utf8":50}],42:[function(require,module,exports){
'use strict';
...
},{"axios":3,"debug":32,"js-base64":47}],43:[function(require,module,exports){
'use strict';
